...
const MENU_EST_HEIGHT = 320 // used only to flip above the trigger near the viewport bottom
...
export function AddToListMenu({ itemId }: AddToListMenuProps) {
  ...
  const reposition = useCallback(() => {
    ...
    const flipUp = r.bottom + 8 + MENU_EST_HEIGHT > window.innerHeight
    const top = flipUp ? Math.max(8, r.top - 8 - MENU_EST_HEIGHT) : r.bottom + 8
    setCoords({ top, left })
  }, [])
        ...
        <AnimatePresence>
          {open && coords && (
            <motion.div
              ...
              className="fixed rounded-xl border border-white/10 shadow-2xl overflow-hidden"
              style={{ top: coords.top, left: coords.left, width: MENU_WIDTH, zIndex: 1000, background: 'var(--color-void)' }}
            >
