...
from athena.core.models import ContentItem, Source  # noqa: E402
...
def view_items(limit=10, source_name=None, category=None):
    with SessionLocal() as session:
        ...
        msg = f"\n--- Showing latest {len(items)} items ---"
        if source_name:
            msg += f" (filtered by source: {source_name})"
        if category:
        ...
        for idx, item in enumerate(items, 1):
            ...
            if item.abstract:
                ...
                abstract = (item.abstract if len(item.abstract) < 150
                            else item.abstract[:147] + "...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--source", type=str,
                        help="Filter by source name (e.g., 'ArXiv')")
    parser.add_argument("-c", "--category", type=str,
                        help="Filter by content category (e.g., 'PAPER', 'COMMUNITY_BLOG')")
