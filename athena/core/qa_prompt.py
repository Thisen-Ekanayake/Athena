...
def build_messages(article_text: str, question: str, history: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    if not history:
        ...
        # We assume history contains all past turns: [{'role': 'user', 'content': '...'}, {'role': 'assistant', 'content': '...'}]
        ...
        # API is stateless, the frontend sends history as [{role: "user", content: "question1"}, {role: "assistant", content: "answer1"}, ...]
...
def prune_history_if_needed(messages: List[Dict[str, str]], max_tokens: int = 8000) -> List[Dict[str, str]]:
    ...
    # Prune oldest turns from history (keep system prompt and turn 1 context if possible, but actually we can't easily prune turn 1 without losing context)
